...
# read all files in directory and define cell types by the name of the csv, cell types are in the name celltype1_celltype2
...
data_path <- "S:/Lab_Member/Tobi/Experiments/Collabs/Neha/clusterProfiler/Datasets/mapped"

# Verify that data_path is a valid directory
if (!dir.exists(data_path)) {
  stop("The specified data_path directory does not exist: ", data_path)
}

# List all CSV files in data_path
csv_files <- list.files(data_path, pattern = "\\.csv$", full.names = TRUE)
if (length(csv_files) == 0) {
  stop("No CSV files found in the data_path directory!")
}

# Extract cell types from the first CSV filename (assumes format: celltype1_celltype2.csv)
file_base <- basename(csv_files[1])
cell_types <- unlist(strsplit(sub("\\.csv$", "", file_base), "_"))

# Use the first CSV file for downstream analysis by updating data_path to point to that file
...
# ----------------------------------------------------
# Process each CSV file in batch
for (current_file in csv_files) {

  # Extract cell types from the filename (assumes format: celltype1_celltype2.csv)
  file_base <- basename(current_file)
  cell_types <- unlist(strsplit(sub("\\.csv$", "", file_base), "_"))

  # ----------------------------------------------------
  # Load and prepare gene data for the current file
  # ----------------------------------------------------

  df <- read.csv(current_file, header = TRUE)
  ...
  top_df <- read.csv(current_file, header = TRUE)
  ...
  # Select top genes by absolute fold change (adjust threshold as needed)
